...
contract MultiSig {
    ...
    function confirmTransaction(uint256 _trasanctionId) public {
        ...
        if(isConfirmed(_trasanctionId) == true){
            _executeTransaction(_trasanctionId);
        }
    }
    ...
    function submitTransaction(address _destination, uint256 _amount, bytes calldata _data) external {
        require(_isOwner() == true, "only owners can call this method");

        uint256 transactionId = _addTransaction(_destination, _amount, _data);
    ...
    function _executeTransaction(uint256 _trasanctionId) private {
    ...
    function _addTransaction(address _destination, uint256 _amount, bytes calldata _data) internal returns (uint256) {
        require(_destination != address(0), "address invalid");

        transactions.push(Transaction(_destination, _amount, false, _data));
